    ...
    for j in range(16): # width
        for i in range(32): # length in pairs
            ...
            if i % 2 == 1:
                ...
                # barrel index is 1024layer + 32width + length (+16 for second half of bytes)

                # second half of pair index is 16 + first half  (0, 16), (1, 17), (2, 18)
                barrelH = 1024 * k + 32 * i + j
                barrelL = 1024 * k + 32 * i + j + 16

                # set barrel values in hex (0-15), k is character in line (0-7 for L, 8-15 for H)
                output[barrelH] = int(instructions[byte0][k + 8]) * offset0 + int(instructions[byte1][k + 8]) * offset1 + int(instructions[byte2][k + 8]) * offset2 + int(instructions[byte3][k + 8]) * offset3
                output[barrelL] = int(instructions[byte0][k]) * offset0 + int(instructions[byte1][k]) * offset1 + int(instructions[byte2][k]) * offset2 + int(instructions[byte3][k]) * offset3
                print(f"Barrel H: {barrelH}, Value: {output[barrelH]:X}")
                print(f"Barrel L: {barrelL}, Value: {output[barrelL]:X}")
            else:
                # L H
                barrelL = 1024 * k + 32 * i + j
                barrelH = 1024 * k + 32 * i + j + 16
